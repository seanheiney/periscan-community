...
- CLAIM: Product LICENSE stays proprietary until founder + counsel
  explicitly flip it. Open-core work is preflight only (matrix, notices,
  SECURITY.md, secret scrub). Option A (Apache-2.0 core + commercial edge)
  is the recommended model, not a completed flip.
  (settled 2026-08-03 plan; 2026-08-14 GA program)
  NOT: change root LICENSE or publish the repo as OSS in a coding session.
  TELL: rewrite `LICENSE` / “we are open source now” → STOP.
