 ...
 * LICENSE flip (root LICENSE stays proprietary until founder + counsel).
 */
...
export const COMMUNITY_EDITION_VALUE_LINE =
  "Community edition is the open-core validation pack: secrets (Gitleaks, detect-secrets, git-secrets, secretlint), SCA (Trivy, OSV, Grype, pip-audit, govulncheck, cargo-audit, retire.js), SAST (Bandit, gosec), IaC (Checkov, Terrascan, KICS, kube-linter, kube-score, Kubescape, Conftest), SBOM (Syft, cdxgen), containers (Dockle, Trivy), TLS (SSLyze/tlsx), ZAP, Nuclei (second mission), Prowler, kube CIS/kube-bench, YARA/Falco rules, MIT recon, nmap/naabu/amass when a runner is enrolled. Not a LICENSE flip. Not live Atomic/Caldera/Metasploit. GPL/LGPL engines stay Engine Lab + license accept.";
